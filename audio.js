/* =========================================================
   ...
   ・Sound … 効果音（正解/不正解/ボタン/かんせい）
   ・Bgm   … BGM。画面で雰囲気を出し分け：
             title … タイトル系（ホーム/選択画面/結果）＝明るく弾むポップ
             game  … パズル中＝あたたかく軽快なグルーヴ
             history … 記録画面＝やさしく振り返るチャイム
   ========================================================= */
...
/* ===================== 効果音 ===================== */
const Sound = (() => {
  let enabled = (localStorage.getItem('soyogi.jigsaw.sound') !== 'off');
  ...
  return {
    ...
    toggle(){
      enabled = !enabled;
      localStorage.setItem('soyogi.jigsaw.sound', enabled ? 'on' : 'off');
      if(enabled) this.tap();
      return enabled;
    },
    unlock(){ audioCtx(); },
    ...
    swap(){ tone(440,0.08,'triangle',0,0.16); },
    fanfare(){ [523,659,784,1046].forEach((f,i)=>tone(f,0.22,'triangle',i*0.12,0.22)); },
  };
})();

/* ===================== BGM（title / game / history） ===================== */
const Bgm = (() => {
  let enabled = (localStorage.getItem('soyogi.jigsaw.bgm') !== 'off');
  ...
  function gameStep(){
    ...
    pluck(ch[s], 0.30, 0.05, 'sine');
    if(s===0){
      pluck(GAME_BASS[ci], 0.6, 0.07, 'sine');
      pad(ch[0], _ac.currentTime+0.02, 1.15, 0.025, 'sine');
      pad(ch[1], _ac.currentTime+0.02, 1.15, 0.02, 'sine');
    }
    if(s===2) pluck(ch[2], 0.26, 0.04, 'sine');
    step++;
  }
  function historyStep(){
    ...
    if(s===0){
      ...
      pluck(ch[0]*2, 0.5, 0.03, 'triangle');
    }
    step++;
  }
  ...
  return {
    ...
    toggle(){
      enabled = !enabled;
      localStorage.setItem('soyogi.jigsaw.bgm', enabled ? 'on' : 'off');
